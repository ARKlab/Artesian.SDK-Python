...
registered = mkdservice.readMarketDataRegistryByName(marketAssessment.providerName, marketAssessment.marketDataName)
if (registered is None):
    registered = mkdservice.registerMarketData(marketAssessment)
    
marketAssessment = MarketData.UpsertData(Artesian.MarketData.MarketDataIdentifier(marketAssessment.providerName, marketAssessment.marketDataName), 'UTC', 
  marketAssessment=
  {
      datetime(2020,1,1):
      {
...
res=query.createMarketAssessment()\
