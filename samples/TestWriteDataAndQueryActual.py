...
registered = mkdservice.readMarketDataRegistryByName(actual.providerName, actual.marketDataName)
if (registered is None):
    registered = mkdservice.registerMarketData(actual)
    
data = Artesian.MarketData.UpsertData(Artesian.MarketData.MarketDataIdentifier(actual.providerName, actual.marketDataName), "UTC",
    rows={
        datetime(2020,1,1,h): 42.0 + h/24.0
        for h in range(0, 23)
    })
